...
import { 
  ...
  Users, ChevronDown, Activity, Radio
} from 'lucide-react';
              ...
              <div className="hidden md:flex items-center gap-2 ml-2 sm:ml-4">
                ...
                <div className="relative font-sans">
                  <button
                    ...
                    className="group inline-flex items-center gap-2 bg-emerald-500/10 dark:bg-emerald-500/15 hover:bg-emerald-500/25 px-3 py-1.5 rounded-full border border-emerald-500/30 hover:border-emerald-500/60 cursor-pointer select-none text-[11px] text-emerald-800 dark:text-emerald-300 font-extrabold shadow-xs transition-all duration-200 active:scale-[0.98] ring-1 ring-emerald-500/15"
                    title="Klik untuk membuka detail IP, MAC, dan Lokasi Pengguna Aktif"
                  >
                    ...
                    <span>
                      <span className="font-mono font-bold text-emerald-700 dark:text-emerald-300">{activeReaders}</span> Pengguna Aktif
                    </span>
                    <ChevronDown className={`w-3 h-3 text-emerald-600/70 dark:text-emerald-400/70 transition-transform duration-200 ${isSessionsDropdownOpen ? 'rotate-180 text-emerald-600 dark:text-emerald-300' : 'group-hover:translate-y-0.5'}`} />
                  </button>
                    ...
                    {isSessionsDropdownOpen && (
                      <>
                        ...
                        <motion.div
                          ...
                          className="absolute left-0 sm:left-auto sm:right-0 lg:left-0 mt-2.5 w-84 sm:w-92 bg-white dark:bg-[#0c0b11] border border-slate-200/90 dark:border-white/10 rounded-2xl shadow-2xl p-4 z-50 text-slate-800 dark:text-white backdrop-blur-xl"
                        >
                          ...
                          <div className="text-[10px] text-slate-500 dark:text-slate-400 mb-2.5 flex items-center justify-between px-1">
                            <span className="flex items-center gap-1">
                              <Activity className="w-3 h-3 text-emerald-500" />
                              <span>Deteksi real-time berkala</span>
                            </span>
                            <span className="font-mono text-[9px] text-slate-400">Heartbeat: 10s</span>
                          </div>

                          <div className="max-h-72 overflow-y-auto space-y-2.5 pr-0.5 custom-scrollbar">
                            {activeSessions.length === 0 ? (
                              <div className="p-6 text-center text-xs text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-white/[0.02] rounded-xl border border-dashed border-slate-200 dark:border-white/5">
                                <Radio className="w-5 h-5 mx-auto mb-2 text-slate-400 animate-pulse" />
                                <p className="font-medium">Memuat data pengguna aktif...</p>
                              </div>
                            ) : (
                              activeSessions.map((session) => {
                                ...
                                return (
                                  <div 
                                    key={session.id} 
                                    className="p-3 rounded-xl bg-slate-50 dark:bg-white/[0.04] border border-slate-200/70 dark:border-white/5 flex flex-col gap-2 hover:bg-slate-100/80 hover:dark:bg-white/[0.07] transition-all"
                                  >
                                    ...
                                    <div className="flex items-center justify-between text-xs font-semibold">
                                      <div className="flex items-center gap-1.5 text-slate-500 dark:text-slate-400 text-[10px] uppercase font-bold tracking-wider">
                                        <Globe className="w-3 h-3 text-emerald-500" />
                                        <span>IP Address</span>
                                      </div>
                                      <button 
                                        onClick={() => handleCopy(session.ip, `ip-${session.id}`)}
                                        className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded bg-white dark:bg-white/5 hover:bg-emerald-500/10 text-slate-800 dark:text-slate-200 hover:text-emerald-600 dark:hover:text-emerald-400 transition text-[11px] font-mono font-bold border border-slate-200/60 dark:border-white/5 cursor-pointer"
                                    ...
                                    <div className="flex items-center justify-between text-xs font-semibold">
                                      ...
                                      <button 
                                        ...
                                        className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded bg-white dark:bg-white/5 hover:bg-blue-500/10 text-slate-800 dark:text-slate-200 hover:text-blue-600 dark:hover:text-blue-400 transition text-[11px] font-mono font-bold border border-slate-200/60 dark:border-white/5 cursor-pointer"
                          ...
                          <div className="mt-3 pt-2.5 border-t border-slate-200 dark:border-white/5 flex items-center justify-between text-[9.5px] text-slate-400">
                            <span>Status: Gateway Aktif</span>
